...
For reference, you can also check the built-in topic list component in the Starlight Sidebar Topics plugin [source code](https://github.com/HiDeoo/starlight-sidebar-topics/blob/main/packages/starlight-sidebar-topics/src/components/Topics.astro) which also relies on the route data object to render the default list of topics.
...
**type:** `Omit<HTMLAttributes<'a'>, 'children' | 'href'> | undefined`

The optional HTML attributes to add to the topic link.
