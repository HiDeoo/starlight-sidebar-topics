...
2. Configure the plugin in your Starlight [configuration](https://starlight.astro.build/reference/configuration/#plugins) in the `astro.config.mjs` file by creating a new topic containing your existing Starlight [`sidebar`](https://starlight.astro.build/reference/configuration/#sidebar) configuration.

   The following example shows how to configure a single topic named "Documentation" linking to the `/docs/` page with your existing sidebar configuration defined using the `items` property:
