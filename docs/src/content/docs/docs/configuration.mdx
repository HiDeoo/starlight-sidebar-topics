...
The link to the topic's content which can be a relative link to local files or the full URL of an external page.
...
**type:** `Omit<HTMLAttributes<'a'>, 'children' | 'href'>`
